<h2>Hey <img src="https://raw.githubusercontent.com/MartinHeinz/MartinHeinz/master/wave.gif" width="30px" height="30px" />, I'm <a href="https://svnair.dev/">Sachin</a>.</h2>
<p>I'm currently a student at <strong><a href="https://asu.edu/">Arizona State University</a></strong>, formerly a <strong><a href="https://xkcd.com/2494/">Data Scientist</a></strong> and currently based in 🏜️ Phoenix. I am working on some side projects, learning a couple new dishes (NOT), trying to continue my hobby of Photography(NOT) and trying to play games on a Macbook.</p>
<p><a href="https://svnair.dev"><img src="https://img.shields.io/badge/-svnair.dev-4E69C8?style=flat-square&amp;labelColor=4E69C8&amp;logo=Firefox&amp;link=https://svnair.dev" alt="Website Badge"></a> <a href="https://www.linkedin.com/in/sachin-v-nair/"><img src="https://img.shields.io/badge/LinkedIn-blue?logo=linkedin;link=https://www.linkedin.com/in/sachin-v-nair/" alt="LinkedIn Badge"></a> </p>
<p>🍌 Don't forget to get some Potassium 🍌</p>
<img align="right" src="https://media1.giphy.com/media/13HgwGsXF0aiGY/giphy.gif" />
<h2>⚡️ A Few Quick Facts</h2>
<ul>
<li>🔭 I’m currently working on <a href="https://github.com/SachinVenugopalan30/JobGlove">JobGlove</a> and <a href="https://xkcd.com/52/">super secret stuff</a>! 🤫</li>
<li>🧐 Learning about <strong>in depth machine learning systems</strong>, <strong>distributed databases</strong>, and a bit of <strong>efficient API development</strong>.</li>
<li><a href="https://github.com/SachinVenugopalan30/leetcode-personal-solutions">I hate leetcode</a> and so do you.</li>
<li>👨‍💻 Most of my projects are available on <a href="https://github.com/SachinVenugopalan30">Github</a>.</li>
<li>💬 Ping me about <strong>python, machine learning, AI, and cloud stuff</strong>.</li>
</ul>
<h2>✒️ Cool stuff I've found</h2>
<details>
    <summary>Explore</summary>
    <li><a target="_blank" href="https://youtu.be/uU06vKlCNXk">Solving Factorio with Terraform</a></li><li><a target="_blank" href="https://www.tigerdata.com/blog/its-2026-just-use-postgres">Just use Postgres for everything, bro</a></li><li><a target="_blank" href="https://proofofcorn.com/">Can AI Grow Corn?</a></li></li>
</details>
<p><a target="_blank" href="https://youtu.be/dQw4w9WgXcQ">Read More</a></p>
<h2>🚀 Some Tools I Use</h2>
<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=python,c,java,js,ts,vscode,linux,bash,git,docker,sklearn,tensorflow,pytorch,postgresql,mongodb,react,nextjs,aws,azure,gcp&perline=5" />
  </a>
</p>
<h2>Stats to show how unproductive I am</h2>
<p align="center">
<a href="https://git.io/streak-stats"><img src="https://streak-stats.demolab.com?user=SachinVenugopalan30&theme=dracula&hide_border=true&short_numbers=true" alt="GitHub Streak" /></a>
</p>
